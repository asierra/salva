class UpdateStartDateAndEndDateIntheses < ActiveRecord::Migration[6.1]
  def up
    Thesis.all.each do |record|
      ...
      record.update(:start_date => start_date, :end_date => end_date)
    end
  end

  def down
    Thesis.all.each do |record|
      record.update(:startmonth => record.start_date.month, :startyear => record.start_date.year, :endmonth => record.end_date.month, :endyear => record.end_date.year)
    end
  end
end
